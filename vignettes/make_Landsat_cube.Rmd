...
use_python("/home/wanda/anaconda3/bin/python", required = TRUE)
py_config()
```
# General
...
# Inputs
```{r}
forcefolder <- Sys.getenv("HOME")#''/home/wanda/Documents/data/force'#Sys.getenv("HOME")#'/home/wanda/Documents/data/force'
...
tiers <- 'T1'#Tier level
sensors <- c('LC08', 'LE07', 'LT05', 'LT04', 'LM05', 'LM04')#sensors
```
...
if(!dir.exists(demfolder)){dir.create(demfolder)}
if(!dir.exists(wvpfolder)){dir.create(wvpfolder)}
...
DEM_NODATA= '-32767', DO_REPROJ = 'TRUE', DO_TILE = 'TRUE',FILE_TILE = 'NULL',TILE_SIZE = '30000',BLOCK_SIZE = '3000', RESOLUTION_LANDSAT = '30', RESOLUTION_SENTINEL2 = '10', ORIGIN_LON = '-25', ORIGIN_LAT = '60', PROJECTION = 'GLANCE7', RESAMPLING = 'CC', DO_ATMO = 'TRUE', DO_TOPO = 'TRUE', DO_BRDF = 'TRUE', ADJACENCY_EFFECT = 'TRUE', MULTI_SCATTERING = 'TRUE', DIR_WVPLUT = 'NULL', WATER_VAPOR = 'NULL', DO_AOD = 'TRUE', DIR_AOD = 'NULL', MAX_CLOUD_COVER_FRAME = '75', MAX_CLOUD_COVER_TILE = '75', CLOUD_THRESHOLD = '0.225', SHADOW_THRESHOLD = '0.02', RES_MERGE = 'IMPROPHE', DIR_MASTER = 'NULL', MASTER_NODATA = '-32767', IMPULSE_NOISE = 'TRUE', BUFFER_NODATA = 'FALSE', TIER = '1', NPROC = '32', NTHREAD = '2', PARALLEL_READS = 'FALSE', DELAY = '3', TIMEOUT_ZIP = '30', OUTPUT_FORMAT = 'GTiff', OUTPUT_DST = 'FALSE', OUTPUT_AOD = 'FALSE', OUTPUT_WVP = 'FALSE', OUTPUT_VZN = 'FALSE', OUTPUT_HOT = 'FALSE', OUTPUT_OVV = 'FALSE'):
    ...
    if(not os.path.isfile(filename)):
        ...
        with fileinput.FileInput(filename, inplace=True, backup='.bak') as file:
            for line in file:
                print(line.replace('FILE_QUEUE = NULL', 'FILE_QUEUE = ' + FILE_QUEUE), end='')
                print(line.replace('DIR_LEVEL2 = NULL', 'DIR_LEVEL2 = ' + DIR_LEVEL2), end='')
                print(line.replace('DIR_LOG = NULL', 'DIR_LOG = ' + DIR_LOG), end='')
                print(line.replace('DIR_TEMP = NULL', 'DIR_TEMP = ' + DIR_TEMP), end='')
                print(line.replace('FILE_DEM = NULL', 'FILE_DEM = ' + FILE_DEM), end='')
                print(line.replace('DEM_NODATA = -32767', 'DEM_NODATA = ' + DEM_NODATA), end='')
                print(line.replace('DO_REPROJ = TRUE', 'DO_REPROJ = ' + DO_REPROJ), end='')
                print(line.replace('DO_TILE = TRUE', 'DO_TILE = ' + DO_TILE), end='')
                print(line.replace('FILE_TILE = NULL', 'FILE_TILE = ' + FILE_TILE), end='')
                print(line.replace('TILE_SIZE = 30000', 'TILE_SIZE = ' + TILE_SIZE), end='')
                print(line.replace('BLOCK_SIZE = 3000', 'BLOCK_SIZE = ' + BLOCK_SIZE), end='')
                print(line.replace('RESOLUTION_LANDSAT = 30', 'RESOLUTION_LANDSAT = ' + RESOLUTION_LANDSAT), end='')
                print(line.replace('RESOLUTION_SENTINEL2 = 10', 'RESOLUTION_SENTINEL2 = ' + RESOLUTION_SENTINEL2), end='')
                print(line.replace('ORIGIN_LON = -25', 'ORIGIN_LON = ' + ORIGIN_LON), end='')
                print(line.replace('ORIGIN_LAT = 60', 'ORIGIN_LAT = ' + ORIGIN_LAT), end='')
                print(line.replace('PROJECTION = GLANCE7', 'PROJECTION = ' + PROJECTION), end='')
                print(line.replace('RESAMPLING = CC', 'RESAMPLING = ' + RESAMPLING), end='')
                print(line.replace('DO_ATMO = TRUE', 'DO_ATMO = ' + DO_ATMO), end='')
                print(line.replace('DO_TOPO = TRUE', 'DO_TOPO = ' + DO_TOPO), end='')
                print(line.replace('DO_BRDF = TRUE', 'DO_BRDF = ' + DO_BRDF), end='')
                print(line.replace('ADJACENCY_EFFECT = TRUE', 'ADJACENCY_EFFECT = ' + ADJACENCY_EFFECT), end='')
                print(line.replace('MULTI_SCATTERING = TRUE', 'MULTI_SCATTERING = ' + MULTI_SCATTERING), end='')
                print(line.replace('DIR_WVPLUT = NULL', 'DIR_WVPLUT = ' + DIR_WVPLUT), end='')
                print(line.replace('WATER_VAPOR = NULL', 'WATER_VAPOR = ' + WATER_VAPOR), end='')
                print(line.replace('DO_AOD = TRUE', 'DO_AOD = ' + DO_AOD), end='')
                print(line.replace('DIR_AOD = NULL', 'DIR_AOD = ' + DIR_AOD), end='')
                print(line.replace('MAX_CLOUD_COVER_FRAME = 75', 'MAX_CLOUD_COVER_FRAME = ' + MAX_CLOUD_COVER_FRAME), end='')
                print(line.replace('MAX_CLOUD_COVER_TILE = 75', 'MAX_CLOUD_COVER_TILE = ' + MAX_CLOUD_COVER_TILE), end='')
                print(line.replace('CLOUD_THRESHOLD = 0.225', 'CLOUD_THRESHOLD = ' + CLOUD_THRESHOLD), end='')
                print(line.replace('SHADOW_THRESHOLD = 0.02', 'SHADOW_THRESHOLD = ' + SHADOW_THRESHOLD), end='')
                print(line.replace('RES_MERGE = IMPROPHE', 'RES_MERGE = ' + RES_MERGE), end='')
                print(line.replace('DIR_MASTER = NULL', 'DIR_MASTER = ' + DIR_MASTER), end='')
                print(line.replace('MASTER_NODATA = -32767', 'MASTER_NODATA = ' + MASTER_NODATA), end='')
                print(line.replace('IMPULSE_NOISE = TRUE', 'IMPULSE_NOISE = ' + IMPULSE_NOISE), end='')
                print(line.replace('BUFFER_NODATA = FALSE', 'BUFFER_NODATA = ' + BUFFER_NODATA), end='')
                print(line.replace('TIER = 1', 'TIER = ' + TIER), end='')
                print(line.replace('NPROC = 32', 'NPROC = ' + NPROC), end='')
                print(line.replace('NTHREAD = 2', 'NTHREAD = ' + NTHREAD), end='')
                print(line.replace('PARALLEL_READS = FALSE', 'PARALLEL_READS = ' + PARALLEL_READS), end='')
                print(line.replace('DELAY = 3', 'DELAY = ' + DELAY), end='')
                print(line.replace('TIMEOUT_ZIP = 30', 'TIMEOUT_ZIP = ' + TIMEOUT_ZIP), end='')
                print(line.replace('OUTPUT_FORMAT = GTiff', 'OUTPUT_FORMAT = ' + OUTPUT_FORMAT), end='')
                print(line.replace('OUTPUT_DST = FALSE', 'OUTPUT_DST = ' + OUTPUT_DST), end='')
                print(line.replace('OUTPUT_AOD = FALSE', 'OUTPUT_AOD = ' + OUTPUT_AOD), end='')
                print(line.replace('OUTPUT_WVP = FALSE', 'OUTPUT_WVP = ' + OUTPUT_WVP), end='')
                print(line.replace('OUTPUT_VZN = FALSE', 'OUTPUT_VZN = ' + OUTPUT_VZN), end='')
                print(line.replace('OUTPUT_HOT = FALSE', 'OUTPUT_HOT = ' + OUTPUT_HOT), end='')
                print(line.replace('OUTPUT_OVV = FALSE', 'OUTPUT_OVV = ' + OUTPUT_OVV), end='')
        return;
makeParFile(r.paramfolder, filename, FILE_QUEUE = FILE_QUEUE, DIR_LEVEL2 = DIR_LEVEL2, DIR_LOG = DIR_LOG, DIR_TEMP = DIR_TEMP, FILE_DEM = FILE_DEM, ORIGIN_LON = '-90', ORIGIN_LAT = '60', RESAMPLING = 'NN', DIR_WVPLUT = DIR_WVPLUT, RES_MERGE = 'REGRESSION',  NPROC = '1', NTHREAD = '1', DELAY = '10', OUTPUT_DST = 'TRUE',  OUTPUT_VZN = 'TRUE', OUTPUT_HOT = 'TRUE', OUTPUT_OVV = 'TRUE')
```
...
queue_file = open("/home/wanda/Documents/data/force/level1/queue.txt", "r")
